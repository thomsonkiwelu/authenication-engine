...
public class LessRangerDivisionConfigSeeder : IBaseSeeder
{
    public async Task SeedAsync(AppDBContext context)
    {
        Logger.LogInformation("Seeding LESS Rangers Division config ...");
        ...
        var hasAnyFields = await context.LessRangerDutyFieldDefinitions.AnyAsync();
        if (hasAnyFields)
            return;
        ...
        var toInsert = new List<LessRangerDutyFieldDefinition>();

        var officerOrder = 1;
        foreach (var f in officerFields)
        {
            toInsert.Add(new LessRangerDutyFieldDefinition
            {
                Category = "officer",
                Label = f.Label,
                Key = f.Key,
        ...
        context.LessRangerDutyFieldDefinitions.AddRange(toInsert);
