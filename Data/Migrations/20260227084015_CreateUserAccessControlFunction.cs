...
namespace conservation_backend.Data.Migrations
{
    ...
    public partial class CreateUserAccessControlFunction : Migration
    {
					...
					BEGIN
						...
						SELECT jsonb_build_object(
							...
							'Roles', COALESCE((SELECT jsonb_agg(
								...
								WHERE ru.""UserId"" = u.""Id"" AND r.""DeletedAt"" IS NULL), '[]'::jsonb
							),
							'Permissions', COALESCE((SELECT jsonb_agg
								...
								JOIN ""RolePermissions"" rp ON ru.""RoleId"" = rp.""RoleId""
								JOIN ""Permissions"" p ON rp.""PermissionId"" = p.""Id""
								WHERE ru.""UserId"" = u.""Id"" AND p.""DeletedAt"" IS NULL), '[]'::jsonb
							),
							'Office',(
								SELECT jsonb_build_object(
									'Id', o.""Id"",
        ...
        {
            migrationBuilder.Sql(@"
				DROP FUNCTION IF EXISTS public.fun_get_user_by_id_with_access_control(uuid);
            ");
        }
    }
}
