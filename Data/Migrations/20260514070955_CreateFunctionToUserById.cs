				...
				AS $BODY$
					...
					BEGIN
						...
						result := jsonb_build_object(
							...
							-- Get all park that are not assigned to user
							'Parks', (
								SELECT COALESCE(jsonb_agg(json_build_object('id', p.""Id"", 'name', p.""Name"")), '[]'::jsonb)  
								FROM ""Parks"" p
								WHERE p.""DeletedAt"" IS NULL
								AND NOT EXISTS (
									SELECT 1 FROM ""UserParks"" up
									WHERE up.""ParkId"" = p.""Id""
									AND up.""UserId"" = userId
								)
							),
							-- Get all system applications that are not assigned to user
							'SystemApplications', (
								SELECT COALESCE(jsonb_agg(json_build_object('id', sa.""Id"", 'name', sa.""Name"")), '[]'::jsonb)  
								FROM ""SystemApplications"" sa
								WHERE sa.""DeletedAt"" IS NULL
								AND NOT EXISTS (
									SELECT 1 FROM ""UserSystemApplications"" us
									WHERE us.""SystemApplicationId"" = sa.""Id""
							...
							'AssignedRoles', (
								WITH assigned_role_row AS (
									SELECT
										...
										sa.""CreatedAt"" as SystemApplicationCreatedAt,
										sa.""CreatedBy"" as SystemApplicationCreatedBy
									FROM ""RoleUsers"" ru
									JOIN ""Roles"" r ON r.""Id"" = ru.""RoleId""
									LEFT JOIN ""Users"" u ON u.""Id"" = ru.""CreatedBy""
								...
								SELECT COALESCE(jsonb_agg(
									json_build_object(
										...
										'Slug', arw.""Slug"",
										'SystemApplicationId', arw.""SystemApplicationId"",
										...
										'SystemApplication', jsonb_build_object(
											...
											'CreatedAt', SystemApplicationCreatedAt,
											'CreatedBy', SystemApplicationCreatedBy
										)
									)
							...
							-- Get all park assigned to user
							'AssignedParks', (
								WITH assigned_park_row AS (
									SELECT
										ROW_NUMBER() OVER (ORDER BY up.""CreatedAt"" DESC) as row_number,
										up.*,
										p.""Id"",
										p.""Name"",
										p.""Code"",
									...
									LEFT JOIN ""Users"" u ON u.""Id"" = up.""CreatedBy""
									WHERE up.""UserId"" = userId
								)
								SELECT COALESCE(jsonb_agg(
									json_build_object(
										'RowNumber', apw.row_number,
										'Id', apw.""Id"",
										'Name', apw.""Name"",
										'Code', apw.""Code"",
										'Zone', apw.""Zone"",
										'CreatedAt', apw.""CreatedAt"",
										'CreatedBy', apw.""Username""
									)
									ORDER BY apw.""CreatedAt"" DESC
								), '[]'::jsonb)
								FROM assigned_park_row apw
							),
							-- Get all system applications assigned to user
							'AssignedSystemApplications', (
								WITH assigned_applications_row AS (
									SELECT
										ROW_NUMBER() OVER (ORDER BY usa.""CreatedAt"" DESC) as row_number,
										usa.*,
										sa.""Id"",
										sa.""Name"",
										u.""Username""
									FROM ""UserSystemApplications"" usa
									JOIN ""SystemApplications"" sa ON sa.""Id"" = usa.""SystemApplicationId""
									LEFT JOIN ""Users"" u ON u.""Id"" = usa.""CreatedBy""
									WHERE usa.""UserId"" = userId
								)
								SELECT COALESCE(jsonb_agg(
									json_build_object(
										'RowNumber', apr.row_number,
										'Id', apr.""Id"",
										'Name', apr.""Name"",
										'CreatedAt', apr.""CreatedAt"",
										'CreatedBy', apr.""Username""
									)
									ORDER BY apr.""CreatedAt"" DESC
								), '[]'::jsonb)
								FROM assigned_applications_row apr
							)
						);
