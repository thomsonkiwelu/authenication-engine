...
public class SightingTurtleRepository(AppDBContext context, IUserContext userContext): ISightingTurtleRepository
{
    private readonly AppDBContext _context = context;
    private readonly IUserContext _userContext = userContext;
    
    public async Task<PagedList<SightingTurtleResponseDto>> GetPagedData(SightingTurtlePaginationDto dto)
    {   
        ...
        await using var command = new NpgsqlCommand(
            "SELECT fn_sighting_turtles(@page, @pageSize, @search, @parkId, @parkIds)",
            connection
        );
        ...
        var apiResponse = JsonSerializer.Deserialize<SightingTurtleSqlResponseDto>(
            jsonResult,
    ...
    public async Task<string> Create(SightingTurtleRequestDto dto)
    {
        ...
        await connection.OpenAsync();
        await using var command = new NpgsqlCommand(
            "SELECT fn_create_sighting_turtle(@data::jsonb)",
            connection
        );
    ...
    public async Task<GetSightingTurtleDto> GetById(Guid id)
    {
        var dataSql = "SELECT fn_sighting_turtle_by_id({0})";
        ...
        if (string.IsNullOrWhiteSpace(jsonString))
            throw new ArgumentNullException("Failure to get sighting turtle data");

        var result = JsonSerializer.Deserialize<GetSightingTurtleDto>(
            jsonString,
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
        );
        return result ?? new GetSightingTurtleDto();
    }

    public async Task<string> Update(Guid id, SightingTurtleRequestDto dto)
    {
        ...
        await connection.OpenAsync();
        await using var command = new NpgsqlCommand(
            "SELECT fn_update_sighting_turtle(@data::jsonb, @Id)",
            connection
        );
        command.Parameters.AddWithValue("data", NpgsqlDbType.Jsonb, dtoToJson);
    ...
    {
        var sightingTurtle = await _context.SightingTurtles.FindAsync(id);
        if (sightingTurtle is null)
            throw new KeyNotFoundException("Failure to get sighting turtle data");

        sightingTurtle.DeletedAt = DateTime.UtcNow;
        sightingTurtle.UpdatedBy = _userContext.GetUserId();
        sightingTurtle.UpdatedAt = DateTime.UtcNow;
